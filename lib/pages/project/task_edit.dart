...
class TaskEditPage extends StatefulWidget {
  ...
  TaskEditPage({required this.task, required this.taskState})
...
class _TaskEditPageState extends State<TaskEditPage> {
                    ...
                    child: ListView(
                      ...
                      children: <Widget>[
                            ...
                            children: [
                              Flexible(
                                ...
                                child: TextFormField(
                                  ...
                                  onSaved:
                                      (repeatAfter) =>
                                          _repeatAfter = getDurationFromType(
                                            repeatAfter,
                                            _repeatAfterType,
                                          ),
                                  onChanged: (_) => _changed = true,
                        ...
                        Padding(
                          ...
                          child: Column(children: _reminderInputs),
                        ),
                        Padding(
  ...
  _saveTask(BuildContext context) async {
    ...
    await VikunjaGlobal.of(
      context,
    ).labelTaskBulkService.update(updatedTask, _labels).catchError((err) {
    ...
    widget.taskState
        .updateTask(context: context, task: updatedTask)
        .then((task) {
          setState(() {
            _loading = false;
            _changed = false;
  ...
  _searchLabel(String query) {
    return VikunjaGlobal.of(context).labelService.getAll(query: query).then((
      labels,
    ) {
  ...
  void _createAndAddLabel(String labelTitle) {
    ...
    VikunjaGlobal.of(context).labelService.create(newLabel).then((
      createdLabel,
    ) {
  ...
  _onColorEdit() {
    _pickerColor =
        _resetColor || (_color ?? widget.task.color) == null
            ? Colors.black
            : _color ?? widget.task.color;
    showDialog(
      context: context,
