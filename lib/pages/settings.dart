...
class SettingsPageState extends State<SettingsPage> {
  ...
  void init() {
    ...
    VikunjaGlobal.of(context).projectService.getAll().then(
      (value) => setState(() => projectList = value),
    );
    ...
    VikunjaGlobal.of(context).settingsManager.getSentryEnabled().then(
      (value) => setState(() => sentryEnabled = value),
    );

    VikunjaGlobal.of(context).settingsManager.getVersionNotifications().then(
      (value) =>
          setState(() => getVersionNotifications = value == "1" ? true : false),
    );

    VikunjaGlobal.of(context).versionChecker.getCurrentVersionTag().then(
      (value) => setState(() => versionTag = value),
    );
    ...
    VikunjaGlobal.of(context).settingsManager.getThemeMode().then(
      (value) => setState(() => themeMode = value),
    );

    VikunjaGlobal.of(context).newUserService?.getCurrentUser().then(
      (value) => {
        setState(() {
          currentUser = value!;
    ...
    return new Scaffold(
      ...
      body: ListView(
        children: [
              ...
              ? CheckboxListTile(
                ...
                onChanged: (value) {
                  ...
                  if (value != null)
                    VikunjaGlobal.of(
                      context,
                    ).settingsManager.setVersionNotifications(value);
                },
              )
              : ListTile(title: Text("...")),
          TextButton(
          ...
          TextButton(
            onPressed:
                () => VikunjaGlobal.of(context).versionChecker
                    .getLatestVersionTag()
          ...
          Text(
            newestVersionTag != null ? "Latest version: $newestVersionTag" : "",
          ),
          Divider(),
          TextButton(
